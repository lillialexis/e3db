...
E3DB has been tested on MacOS, Windows, and Linux:
 ...
 1. Download [e3db-0.7.1.zip](https://github.com/tozny/e3db/releases/download/0.7.1/e3db-0.7.1.zip) and unzip it. Or clone this repo if you want extra fun.
 1. The `e3db` binary is now in `./e3db-0.7.1/bin`. Add that to your path.
...
To obtain the E3DB CLI binary, download [e3db-0.7.1.zip](https://github.com/tozny/e3db/releases/download/0.7.1/e3db-0.7.1.zip).
...
`e3db-0.7.1/bin/e3db`. For ease of use, add this directory to your
path. For example:

    $ unzip e3db-0.7.1.zip -d $HOME
    $ export PATH=$PATH:$HOME/e3db-0.7.1/bin
